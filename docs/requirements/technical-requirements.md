...
| TR-017 | FR-001 | Implement Qualified Dividends & Capital Gain Tax Worksheet with 0%/15%/20% preferential rates, Additional Medicare Tax (Form 8959), and QBI deduction. | `src/financial_manager/data/capital_gains_rates.py`, `src/financial_manager/engine/calculator.py` | `tests/test_2024_validation.py` | Implemented |
| TR-018 | FR-020 | Implement multi-folder document intake scanner with FolderConfig, skip patterns, and folder-specific classification rules for House/ and Condo/ folders. | `src/financial_manager/engine/intake.py` | `tests/test_intake_pipeline.py` | Implemented |
| TR-019 | FR-021 | Implement specialized extractors for settlement statements, solar agreements/receipts, consolidated 1099s, 1099-K, and Roth 1099-R with fallback regex patterns. | `src/financial_manager/engine/extractors.py` | `tests/test_intake_pipeline.py` | Implemented |
| TR-020 | FR-022 | Implement tax picture assembler that routes extracted data to structured sections (income, deductions, credits, withholding, real estate) and identifies gaps. | `src/financial_manager/engine/assembler.py` | `tests/test_intake_pipeline.py` | Implemented |
| TR-022 | FR-023 | Implement financial data import connectors for CSV (Fidelity, generic bank) and OFX/QFX formats with auto-detection, and a data mapper producing tax-relevant summaries. | `src/financial_manager/connectors/` | `tests/test_csv_importer.py`, `tests/test_ofx_importer.py`, `tests/test_data_mapper.py` | Implemented |
| TR-023 | FR-021, FR-022, FR-023 | Implement pipeline API that orchestrates document assembly, financial imports, and tax calculation into a single dashboard endpoint. | `src/financial_manager/api/pipeline_routes.py` | `tests/test_pipeline_routes.py` | Implemented |
